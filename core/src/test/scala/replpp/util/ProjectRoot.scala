...
object ProjectRoot {
  ...
  def relativise(path: String): Path =
    Paths.get(s"./$find/$path")

  def find: Path = {
    val fileThatOnlyExistsInRoot = ".git"

    @tailrec def loop(depth: Int): Path = {
      val rootDirCandidate = Paths.get("../" * depth)

      if (Files.exists(rootDirCandidate.resolve(fileThatOnlyExistsInRoot)))
        rootDirCandidate
